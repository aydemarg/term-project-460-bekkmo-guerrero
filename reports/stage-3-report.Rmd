...
cv_model_comp <- read_csv("../output/tables/cv_model_comparison_wide.csv", col_names = TRUE)
cv_model_comp_clean <- cv_model_comp[-1, ]
cv_model_comp_clean <- cv_model_comp_clean[-c(5:nrow(df)), ]

kable(cv_model_comp_clean,
      digits = 3,
...
Random Forest achieved the lowest grouped CV RMSE of 0.834, outperforming Lasso (0.861), Linear Regression (0.960), and the mean baseline (0.990).
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
test_set_metrics <- read_csv("../output/tables/test_set_metrics.csv", col_names = TRUE)
test_set_metrics_clean <- test_set_metrics[-1, ]
test_set_metrics_clean <- test_set_metrics_clean[-c(4:8), ]

kable(test_set_metrics_clean,
      digits = 3,
...
The pair of figures below plot test-set predictions against actuals using the Random Forest model. The first is on the log scale (how the model was trained and evaluated); the second is back-transformed to the original medal count scale for interpretation. Points clustered tightly around the dashed 45° line indicate good predictions.

![](../output/figures/fig17_pred_vs_actual_medals.png)

For comparison, the following two pairs of figures below plot test-set predictions against actuals using the Linear and Lasso regression models. **Add more interpretation**
...
The baseline model predicts the same average medal outcome for every country. It provides a benchmark to determine whether more sophisticated models extract useful signal from the predictors. **Add more**

#### Linear Regression interpretation

Linear regression assumes a linear relationship between predictors and medal outcomes. It is interpretable and serves as a benchmark parametric model. **Add more**

#### Lasso Regression interpretation

Lasso adds an L1 penalty that shrinks coefficients toward zero, helping reduce overfitting and perform variable selection when predictors are numerous or correlated. **Add more**

### Random Forest interpretation

Random Forest combines many decision trees to capture nonlinear relationships and interactions automatically. It is more flexible than linear models but less interpretable. **Add more**
